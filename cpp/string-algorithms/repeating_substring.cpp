...
int n, power[N], inverse_power[N], prefix_hash[N], suffix_array[N], c[N], cc[N], lcp[N];
string s;
...
int32_t main() {
    ...
    int max_lcp = 0;
    int max_lcp_suffix_index = -1;

    for (int i = 1; i < n; i++) {
        if (lcp[i] > max_lcp) {
            max_lcp = lcp[i];
            max_lcp_suffix_index = suffix_array[i];
        }
    }

    if (max_lcp_suffix_index == -1) {
        cout << -1 << endl;
    }
    else {
        for (int i = 0; i < max_lcp; i++) {
            cout << s[max_lcp_suffix_index + i];
        }
        cout << endl;
    }
